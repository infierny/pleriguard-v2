...
# AWS noise filter - exclude internal/test domains
def is_aws_noise(domain, full_url):
    """Filter out AWS internal/test domains that aren't phishing targets"""
    aws_noise = ['canary', 'vpce-', 'mrap.accesspoint', 's3-global.amazonaws', 
                'internal.aws', 'test.amazonaws', '.internal.', 
                'elasticloadbalancing.amazonaws', 'amazongamelift.com']
    
    for noise in aws_noise:
        if noise in full_url.lower():
            return True
    return False
    ...
    def _do_flush():
        ...
        with get_conn() as conn:
            ...
            for raw_url in batch:
                # Skip AWS and cloud provider domains immediately (DISCRIMINANT)
                raw_url_lower = raw_url.lower()
                cloud_patterns = [
                    # AWS patterns
                    'amazonaws.com', 'amazonaws.eu', 'amazonaws.com.cn', 
                    'amazongamelift.com', 'vpce.amazonaws.com',
                    'amazon.dev', 'amazon-ah.com',
                    # Azure patterns  
                    'azurecontainerapps', 'azure.com', 'cloudapp.azure.com',
                    'azurecontainerapps.io', 'azurecontainerapps.us',
                    # Google patterns
                    'googleapis.com', 'google.com', 'googleusercontent.com',
                    # Admin/Microsoft patterns
                    'admin-mcas'
                ]
                # More aggressive check - check if any cloud pattern is in the URL
                is_cloud_domain = False
                for pattern in cloud_patterns:
                    if pattern in raw_url_lower:
                        is_cloud_domain = True
                        break
